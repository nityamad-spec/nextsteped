...
interface Props {
  ...
  /** Render the course title/meta header row. Consumers embedding the view
   *  in their own header (e.g. a Dialog) can turn this off. */
  showHeader?: boolean;
}
...
interface Stats {
  ...
  examCompletedOne: StudentLite[];
  examNotStarted: StudentLite[];
  chatStudents: number;
  chatMessages: number;
}
...
const CourseAnalyticsView = ({ course, showHeader = true }: Props) => {
  ...
  type RosterView = "done" | "pending" | "active" | "dormant" | "completed" | "not-completed" | "quiz-completed" | "quiz-partial" | "quiz-not-started" | "exam-completed" | "exam-one" | "exam-not-started" | "mastery-beginner" | "mastery-developing" | "mastery-proficient" | "mastery-expert" | "mastery-none";
  ...
  const load = useCallback(async (courseId: string, showSkeleton: boolean) => {
    ...
    async function fetchAllRange<T>(
      ...
      for (let from = 0; ; from += PAGE) {
        ...
        if (error) {
          console.error("CourseAnalyticsView paginated fetch error", error);
          break;
        }
        const chunk = data ?? [];
        out.push(...chunk);
        ...
        <DialogContent className="max-w-md max-h-[75vh] flex flex-col">
          {(() => {
            ...
            const cfg: Record<RosterView, { title: string; list: ReadonlyArray<StudentLite & { done?: number; remaining?: number }>; desc: (n: number) => string }> = {
              ...
              "exam-completed": { title: `Completed all ${stats?.examsTotal ?? 0} exams`, list: stats?.examCompletedAll ?? [], desc: (n) => `${n} students completed every active exam.` },
              "exam-one": { title: "Completed ≥1 exam", list: stats?.examCompletedOne ?? [], desc: (n) => `${n} students completed at least one active exam.` },
              "exam-not-started": { title: "Not started exams", list: stats?.examNotStarted ?? [], desc: (n) => `${n} enrolled students have not submitted any active exam.` },
